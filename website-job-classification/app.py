    ...
    if 'username' in session:
        if request.method == 'POST':
            ...
            if request.form['button'] == 'Classify':
                ...
                jobsFT = FastText.load("jobAdFT.model")
                jobsFT_wv = jobsFT.wv
                ...
                elif cat_recommend not in ['accounting', 'engineering', 'healthcare', 'hospitality', 'it', 'pr', 'sales', 'teaching']:
                    return render_template('admin.html', prediction=cat_recommend,
                                        title=f_title, description=f_content,
                                        category_flag='Recommended category must belong to: accounting, engineering, healthcare, hospitality, it, pr, sales, teaching')
                    ...
                    for filename in sorted(os.listdir(os.path.join(dir_path, folder))):
                        if filename.endswith('html'):
                            with open(os.path.join(dir_path, folder, filename), encoding="utf8") as file:
                                ...
                                if search_string in file_content or (search_string + "s") in file_content or (search_string + "ing") in file_content :
